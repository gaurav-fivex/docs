...
2. Click on +CREATE SERVICE ACCOUNT
    ...
    ![st-up10](/images/set-up6.png)


3. Fill in the Service account details
4. Click on CREATE AND CONTINUE

     ![set-up7](/images/set-up7.png)
...
Use the created service account credentials (JSON file) to import your Google BigQuery instance into 5X.
  ...
  ![set-up7](/images/bigquery_import.png)
...
## Add your team
Navigate to [Team](/team) and select the **Add a User** button on the top-right corner. Assign the user’s [5X role ](/Team/rbac) and their warehouse roles
...
[App connections](/Settings/app-connections) let you define the destination for Ingestion, Jobs and BI capabilities in the deployment environment. 

Ensure you have the destination configured to access the capabilities on your workspace. 
1. Verify the [Ingestion connection](/Settings/app-connections) to ensure the Ingestion destinations (default database) are configured. 
2. Verify the [Job connection](/Settings/app-connections) to ensure the orchestration destinations (default warehouse) are configured.
3. Verify the [BI connection](/Settings/app-connections) to ensure the BI destinations (default warehouse and database) are configured.

<Note>
- A default ingestion connection is pre-configured if you provision a new warehouse from 5X. Check [**App connections**](/Settings/app-connections) for more details. 
...
<p> You can use the IDE to execute SQL queries for your warehouse by default. Refer to [Credentials](/Settings/credentials) to enable dbt access on the IDE. </p>

## Admin-only settings
Admins have access to the following pages to manage their workspace end-to-end. 
1. Manage your billing cycles from the [Billings](/Settings/billing) page. 
